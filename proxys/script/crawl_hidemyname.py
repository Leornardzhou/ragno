...
def get_proxy_per_page(driver, page_id):
    ...
    out = dict()
    url = 'https://hidemy.name/en/proxy-list/?start={}#list'.format((page_id-1)*64)
    qprint('get page {}'.format(url))
    driver.get(url)
    
    for tr in driver.find_elements_by_xpath('//div[@id="content-section"]//table/tbody/tr'):
        tmp = []
        ...
        value = {
            'country_city': tmp[2], 'speed': tmp[3],
            'type': tmp[4], 'anonymity': tmp[5], 
            'last_check': tmp[6],            
        }
        out[key] = value
...
def crawl_hidemyname(out_json):

    options = webdriver.ChromeOptions()
    options.add_argument('headless')
    driver = webdriver.Chrome(chrome_options=options)

    driver.get('https://hidemy.name/en/proxy-list/?start=0#list')
    while True:
        n = len(driver.find_elements_by_xpath('//div[@id="content-section"]//table/tbody/tr'))
        if n:
            break
        qprint('wait until the page is loaded')
        time.sleep(2)
