...
def get_proxy_per_page(driver, page_id):
    ...
    url = 'https://premproxy.com/list/{0:02d}.htm'.format(page_id)
    ...
    for tr in driver.find_elements_by_xpath('//tbody/tr'):
        ...
        if tmp[0].endswith(':'):
            continue
        tmp = [x.split('\n')[1] if '\n' in x else x for x in tmp]
        key = tmp[0]
        value = {
            'anonymity': tmp[1], 'last_check': tmp[2], 'country': tmp[3], 
            'city': tmp[4], 'isp':tmp[5],
        }
        out[key] = value
